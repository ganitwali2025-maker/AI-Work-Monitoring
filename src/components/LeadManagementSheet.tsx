  ...
  return (
    <div className="bg-[#F8F9FD] h-full w-full flex flex-col font-sans min-w-0 min-h-0 overflow-hidden">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col flex-1 min-h-0 min-w-0 relative">
        ...
        {/* Orange Header Area */}
        <div className="bg-gradient-to-r from-[#e09163] to-[#c2703f] pt-6 pb-12 px-8 text-white relative shrink-0">
            ...
            return (
              <button 
                ...
                className={`flex items-center gap-2 px-5 py-3 rounded-t-xl transition-all shadow-sm cursor-pointer whitespace-nowrap font-bold text-sm ${
                  isActive 
                    ? 'bg-white text-[#c2703f] border-t border-x border-gray-200' 
                    : 'bg-[#F4F5F9] text-gray-500 hover:bg-gray-100 hover:text-gray-700 border border-transparent'
                }`}
              >
                <span className={isActive ? 'text-[#c2703f]' : 'text-gray-400'}>{tab.icon}</span>
                {tab.name}
              </button>
            );
          })}
        ...
        <div className="bg-white flex-1 flex flex-col -mt-[1px] border-t border-gray-200 min-h-0">
          ...
          <div className="px-8 py-5 border-b border-gray-100 shrink-0">
            <div className="relative max-w-sm">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-3 md:py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#c2703f]/20 focus:border-[#c2703f] transition-all bg-gray-50 hover:bg-white text-gray-800"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-auto scrollbar-none">
            <table className="w-max text-left text-sm whitespace-nowrap border-separate border-spacing-0">
              <thead className="text-white font-bold text-[11px] uppercase tracking-wider sticky top-0 bg-[#c2703f] z-20 shadow-sm">
                <tr>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329] sticky left-0 z-30 bg-[#c2703f] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.3)]">Lead ID</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Lead Date</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Company Name</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Industry Type</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Contact Person</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Designation</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Mobile Number</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Email Address</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">City</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">State</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Lead Source</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Product Interest</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Lead Value (₹)</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Priority Level</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Lead Status</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Assigned To</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Assign Date</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Last Follow-up Date</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Last Followup DoneBy</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Last Followup Summary</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Total Follow-ups</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Next Action</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Next Action Date</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Expected Closure Date</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Conversion Prob (%)</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Quotation Sent (Y/N)</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">PO Received (Y/N)</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Lead Age (Days)</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Remarks</th>
                  <th className="px-6 py-4 font-bold border-b border-[#9c5329]">Closed Date</th>
                </tr>
              </thead>
              <tbody className="text-gray-700">
                {leads.map((lead, idx) => {
                  ...
                  return (
                    <tr key={lead.id} className={`group cursor-pointer ${rowBg} hover:bg-orange-50 transition-colors`}>
                      <td className={`px-6 py-3 border-b border-gray-100 font-bold text-[#c2703f] sticky left-0 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] transition-colors ${isEven ? 'bg-white group-hover:bg-orange-50' : 'bg-gray-50 group-hover:bg-orange-50'}`}>{lead.leadId}</td>
